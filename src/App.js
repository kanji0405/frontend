...
//====================================================================================
class APIManager {
    static parseUrl(name) {
        return this.BaseUrl + name;
    }
    static getAPI(name) {
        return new Promise((resolve, reject) => {
            try {
                fetch(this.parseUrl(name), {
...
APIManager.BaseUrl = "https://opendata.resas-portal.go.jp/api/";
...
class PrefController {
    ...
    get isChecked() {
        const el = document.querySelector(
            `input[name='prefs'][value=${this.id}]`,
        );
        ...
        return (
            <label key={this.id + "_label"}>
                <input
                    type="checkbox"
                    name="prefs"
                    onChange={this.requestUpdate.bind(this)}
                    value={this.id}
                />
...
PrefController.ColorList = ["red", "blue", "green", "purple", "orange", "gray"];
    ...
    formatData(prefs) {
        ...
        prefs.forEach((pref) => {
            ...
            popl.data[0].data.forEach((json) => {
                ...
                let yearObject = this.graphData.find(
                    (obj) => obj.year === year,
                );
...
export default class MainView extends React.Component {
    ...
    componentDidMount() {
        if (this._prefControllers.length > 0) {
            return;
        }
            ...
            <article className="poplModelArticle">
                ...
                <div className="poplModelContents">
                    ...
                    <div className="poplModelPrefs">
                        {this._prefControllers.map((pref) =>
                            pref.renderCheckbox(),
                        )}
                    </div>
                    <hr />
